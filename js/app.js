(() => {
    ...
    (() => {
        ...
        function initSliders() {
            if (document.querySelector(".cocktails__body")) new Swiper(".cocktails__body", {
                ...
                navigation: {
                    prevEl: ".cocktails-btn-prev",
                    nextEl: ".cocktails-btn-next"
                },
                breakpoints: {
                    320: {
                        slidesPerView: 2,
            ...
            if (document.querySelector(".cuisine__body")) new Swiper(".cuisine__body", {
                modules: [ Pagination ],
                observer: true,
                observeParents: true,
                slidesPerView: 1,
            ...
            if (document.querySelector(".gallery-hero__body")) new Swiper(".gallery-hero__body", {
                modules: [ Autoplay ],
                observer: true,
                observeParents: true,
                slidesPerView: 1,
            ...
            if (document.querySelector(".delivery__body")) new Swiper(".delivery__body", {
                modules: [ Autoplay ],
                observer: true,
                observeParents: true,
                slidesPerView: 1,
            ...
            if (document.querySelector(".item-bar__body")) {
                const sliders = document.querySelectorAll(".item-bar__body");
                sliders.forEach((slider, index) => {
                    const nextBtn = slider.closest(".item-bar")?.querySelector(".swiper-button-next");
                    const prevBtn = slider.closest(".item-bar")?.querySelector(".swiper-button-prev");
                    new Swiper(slider, {
                        modules: [ Navigation ],
                        observer: true,
                        observeParents: true,
...
***************************************************************************** */
        ...
        (function initGalleries() {
            ...
            galleries.forEach(gallery => {
                prepareGalleryMarkup(gallery);
                if (gallery._lgInstance) {
                    try {
                        gallery._lgInstance.destroy(true);
                    } catch (e) {}
                    gallery._lgInstance = null;
                }
                const lg = lightgallery_es5(gallery, {
                ...
                gallery._lgInstance = lg;
                instances.push({
                    gallery,
                    instance: lg
                });
            });
            if (window.flsModules) window.flsModules.gallery = instances;
            function prepareGalleryMarkup(root) {
                const anchors = root.querySelectorAll("a");
                if (anchors.length) {
                    anchors.forEach(a => a.setAttribute("data-lg-item", ""));
                    return;
                }
                const items = root.querySelectorAll("img, picture img, .ibg img");
                items.forEach(img => {
                    const full = img.dataset.full || img.currentSrc || img.src;
                    const caption = img.alt || img.closest("[data-caption]")?.dataset.caption || "";
                    const a = document.createElement("a");
                    a.href = full;
                    a.setAttribute("data-lg-item", "");
                    if (caption) a.setAttribute("data-sub-html", caption);
        ...
        document.addEventListener("click", e => {
            const btn = e.target.closest(".compound__btn");
            if (!btn) return;
            const compound = btn.closest(".compound");
            if (!compound) return;
            document.querySelectorAll(".compound._active").forEach(el => {
                if (el !== compound) el.classList.remove("_active");
            });
            compound.classList.toggle("_active");
        });
        function initStickyObserver() {
            const mediaQuery = window.matchMedia("(min-width: 991.98px)");
            function handleObserver() {
                if (window._stickyObservers) window._stickyObservers.forEach(obs => obs.disconnect());
                window._stickyObservers = [];
                if (!mediaQuery.matches) {
                    document.querySelectorAll(".item-bar__gallery-sticky._off-sticky").forEach(el => {
                        el.classList.remove("_off-sticky");
                    });
                    return;
                }
                document.querySelectorAll(".item-bar").forEach(bar => {
                    const sticky = bar.querySelector(".item-bar__gallery-sticky");
                    const trigger = bar.querySelector(".item-bar__gallery-small");
                    if (sticky && trigger) {
                        const observer = new IntersectionObserver(entries => {
                            entries.forEach(entry => {
                                if (entry.isIntersecting) sticky.classList.add("_off-sticky"); else sticky.classList.remove("_off-sticky");
                            });
                        }, {
                            root: null,
                            threshold: .1
                        ...
                        window._stickyObservers.push(observer);
                    }
                });
            }
            handleObserver();
            mediaQuery.addEventListener("change", handleObserver);
        }
        window.addEventListener("DOMContentLoaded", initStickyObserver);
        document.addEventListener("DOMContentLoaded", () => {
            const heroVideo = document.querySelector(".hero__video");
            if (!heroVideo) return;
            const videoPc = heroVideo.querySelector(".hero__video-bg_pc");
            const videoMob = heroVideo.querySelector(".hero__video-bg_mob");
            const activeVideo = window.matchMedia("(max-width: 768px)").matches ? videoMob : videoPc;
